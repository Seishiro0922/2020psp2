...
## ソースコードの説明
6  標本の平均を求める関数   kokokokkokkokokokok
...
[comment #20200521]
- 標本平均，標本分散，母集団平均，母集団分散の答えが間違っています．README.mdに正しい答えがあるので，この答えにならなければ，プログラムが間違っています．
- [k01.c#L6]からのave_onlineですが，引数で受け取ったaveを使わずにave=1としていますが....
  - [k01.c#L11]からのvar_onlineでも同様で，引数で受け取ったsaveやvar,veを使わずに関数の中でこれらを初期化しているようですが，，，，
- 変数名　A1 とか　A2　という名前はやめてください．きちんとした名前にするように．

[comment #20200527]
- 標本平均の求め方はデータを増やす毎に平均(ave)を更新する手法となっています。したがって、main関数内にあるaveの初期値は0とすることをお勧めします。
- 標本平均は((N-1)*x_N-1+x_N)/Nで解くことができます。このプログラムでは恐らくiがNの事を、aveがx_N-1の事を、valがx_Nの事を指しています。ですので(i-1)は計算には不要です。
- 標本分散のプログラムvarでの((i-1)/i*save)の()のつけ方が間違っています。恐らく、i-1/iにsaveを掛けたいのでしょうが、この書き方だとi-1をi*saveで割ってしまいますので修正をお願いします。
- var,save,veは初期化されているわけではないので引数として扱う事はできません。引数として使用したいのならば、main関数内で何かしら初期値を代入する必要があります。ちなみに最終的にvarで返したいのならばvarは引数として扱う必要はないです。
- 恐らくveはaveと全く同じものなので、そのままaveを使用して大丈夫だと思われます。
- save=(i-1)*save/i+val*val/iの式はsave=((i-1)*save+val*val)/iと書く事をお勧めします。
- varは関数内で一番最後に書かなければいけません。saveの値が不明となっているので計算ができなくなります。
- 
[comment #20200603]
- ave_online()関数の先頭にaveを標準入力で出力してみると、aveが更新されず、0のままでした。ave_online()関数内でのaveは更新されるけど、main関数内でaveが更新される機会がないのでi=2になった時もave=0のままです。main関数内のaveをave_online()に持っていく仕組みなので、main関数内でaveの更新の機会を与えるか、ave_online()関数の第3引数を他の変数に変えるといいでしょう。
- var_online()関数も同様に、main関数内のsaveが更新されずにvar_online()関数へと引っ張られるので計算が上手くいかないのだと思います。saveもmain関数内で更新する機会を与えるか、var_online()関数の引数を変えるといいでしょう。
- 標本分散の計算は一番最後に計算されれば大丈夫ですので、var_online()の引数にaveは必要ありません。
- return()の中でさらに計算を行っているのでsamplevarianceの値がとても大きくなるのだと思いますので、今回はreturnの中で計算を行わず、returnの中は変数のみにしましょう。
- samplevarianceを求める時に式に注目してみて下さい。何かの変数と一致するはずなので、なるべく簡単な式にすると値がとても大きくはなりません。
