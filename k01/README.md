...
- 9,10  for分を用いて平均の関数を求める。　　　　　　　　　　　　　
- 18　　分散を求める公式
- 19　　標本の二乗の平均を求める式
- 20　　平均を求める式
- 34　　標本の数を指定
- 50,51    関数を用いて平均と分散を求める。   
...
出力　ave=1.000000    var=0.000000

## 修正履歴

[comment #20200507]
- コンパイルはできて実行ファイルが作成されますが，実行ファイルが正常に動きません．
  `./k01`を実行すると，ソースコードによると標本サイズを入力するのかな．`8`を入力してみました．
  ```
  8
  input the filename of sample:the filename of sample:
  File open error
  ```
  `8`を入力すると，ダダダッと何も尋ねられずにFile open errorで異常終了します．正常に動くようにしましょう．
- main関数中のwhile文は，指定されたcsvファイルを1行ずつ読むことを繰りかえすループになっています．
  読み取れなくなったら（ファイルの最後までいったら）ループを抜けます．
  読み取れた場合には，その1行をscanfして数値の部分をvalに読み取り，...という感じです．
- whileループの中で，ave_onlineやvar_onlineに渡す`i`は何が入っているのでしょうか．関数のカッコの中は，すべて数値です．変数名が渡されるわけではありません．いまのソースコードだと，たとえば50行目は，whileの3回目のループでは，
  - ../sample/heights_male.csvの3行目の173.62がvalに入っていて，
  - iは初期化されていないし上書きもされないから仮に0が入っているとし，
  - aveも初期化されていないし上書きされていないから仮に0が入っているとし，
  - nは最初にscanfされる値だから8かな．
  ということで，
  ```
  A1 = ave_online(0, 173.62, 0, 8) ;
  ```
  が実行されます．これを関数ave_online側では，iに0，valに173.62，aveに0，nに8が設定されて関数が動きます．ここでのi,val,ave,nはmain関数のそれらと名前は同じでも，異なるメモリです．関数の中でi=0~8までループしていますが，この計算がよくわかりません．ループの中でvalがいつでも同じだから，aveも更新されない気がします．よくわかりません．
- whileループの中で行うことは，1行ずつ数値を読むたびに，`ave=ave_online(val,ave,n)`と`square_ave=ave_online(val*val,square_ave,n)`でvalの平均aveとval*valの平均square_aveを更新していく作業になるはずです．

- レポートのコードの説明を読んでも，コードを実際に読んでも，プログラムの流れがつかめません．中岡くんもわかっていないのではないでしょうか．
  ところどころに`printf`を使って，その行を通過するときの変数の値を表示させたりして，プログラムの流れをイメージしてみてください．
